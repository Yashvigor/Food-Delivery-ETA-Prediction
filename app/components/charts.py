...
# Custom Dark-Minimalist (SaaS Slate) theme layout settings for Plotly
THEME_LAYOUT = dict(
...
def render_daily_deliveries_chart(orders_df: pd.DataFrame):
    ...
    fig = go.Figure()
    # Add Bar for total orders (Soft Swiggy Orange)
    ...
    # Add Line for avg ETA (Zomato Red)
    fig.add_trace(go.Scatter(
        x=daily_stats['Date'],
        y=daily_stats['Avg_ETA'],
        name="Avg ETA (Mins)",
        line=dict(color="#ff2b54", width=3, shape='spline'),
        marker=dict(size=8, color="#ff2b54"),
        yaxis="y2"
    ))
    ...
    layout_opts.update(dict(
        title="Daily Delivery Volume & Average Times",
        yaxis=dict(
        ...
        yaxis2=dict(
            title=dict(
                text="Average Delivery Time (Mins)",
                font=dict(color="#ff2b54")
            ),
            overlaying="y",
            side="right",
            showgrid=False,
            tickfont=dict(color="#ff2b54")
        ),
...
def render_traffic_impact_chart(orders_df: pd.DataFrame):
    ...
    fig = px.bar(
        ...
        y="actual_eta",
        color="Traffic_Level",
        # Clean modern SaaS gradient progression (Green -> Blue -> Orange -> Red)
        color_discrete_sequence=["#10b981", "#3b82f6", "#ff9f43", "#ef4444"],
        title="How Traffic Slows Down Delivery Times"
    )
...
def render_weather_delay_chart(orders_df: pd.DataFrame):
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(
            ...
            # Slate green to vibrant rose
            colorscale=[[0, '#10b981'], [0.5, '#facc15'], [1.0, '#ff2b54']],
            showscale=False
        ),
        name="Delay Rate %"
    ))
    ...
    layout_opts = THEME_LAYOUT.copy()
    layout_opts.update(dict(
        title="How Weather Affects Delay Rates",
...
def render_courier_experience_vs_time(orders_df: pd.DataFrame):
    ...
    fig = px.scatter(
        ...
        title="Rider Experience vs Delivery Time",
